...
class RecordRepositoryImplTest {
    ...
    @Mock
    private OperationEntityJpaRepository operationEntityJpaRepository;
    @Mock
    private RecordEntityJpaRepository recordEntityJpaRepository;
    @InjectMocks
    ...
    void setUp() {
        ...
        Mockito.when(operationEntityJpaRepository.findByType(Operations.SQUARE_ROOT)).thenReturn(Optional.of(operationEntity));
    ...
    void save() {
        ...
        Mockito.verify(operationEntityJpaRepository, Mockito.times(1)).findByType(Operations.SQUARE_ROOT);
    ...
    void notFoundUser() {
        ...
        recordRepository.save(record);
        Mockito.verify(userEntityJpaRepository, Mockito.times(1)).findByEmail("email");
        Mockito.verify(operationEntityJpaRepository, Mockito.times(0)).findByType(Operations.SQUARE_ROOT);
        Mockito.verify(recordEntityJpaRepository, Mockito.times(0)).save(Mockito.any());
    }

    @Test
    void notFoundOperation() {
        Mockito.when(operationEntityJpaRepository.findByType(Operations.SQUARE_ROOT)).thenReturn(Optional.empty());
        recordRepository.save(record);
        Mockito.verify(userEntityJpaRepository, Mockito.times(1)).findByEmail("email");
        Mockito.verify(operationEntityJpaRepository, Mockito.times(1)).findByType(Operations.SQUARE_ROOT);
        Mockito.verify(recordEntityJpaRepository, Mockito.times(0)).save(Mockito.any());
