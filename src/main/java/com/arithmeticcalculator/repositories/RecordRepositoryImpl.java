...
public class RecordRepositoryImpl implements RecordRepository {
    ...
    @Autowired
    public RecordRepositoryImpl(UserEntityJpaRepository userEntityJpaRepository, OperationEntityJpaRepository operationEntityJpaRepository, RecordEntityJpaRepository recordEntityJpaRepository) {
    ...
    public <T> void save(@NonNull Record<T> record) {
        userEntityJpaRepository.findByEmail(record.getUser().getEmail())
                .ifPresent(userEntity -> operationEntityJpaRepository.findByType(record.getOperation().getOperations())
                        .ifPresent(operationEntity -> {
                            var recordEntity = RecordEntity.builder()
                                    .date(record.getDate())
                                    .user(userEntity)
